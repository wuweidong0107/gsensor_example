...
{
    printf("Usage: gsensor [OPTION...]\n");
    printf("    -l, --gsensor     char device path\n");
    printf("    -i, --input           input device path\n");
    printf("    -e, --enable          enable device\n");
    printf("    -r, --read            read event\n");
    printf("    -h, --help            show this help message\n");
    printf("Example:\n");
    printf("    gsensor -l /dev/mma8452_daemon -i /dev/input/by-path/platform-fe5e0000.i2c-event -e 1 -r\n");
    exit(0);
}
...
int main(int argc, char** argv)
{
    ...
    if (ls == NULL) {
        printf("gsensor_new() fail\n");
        return -1;
    }

    if (gsensor_open(ls, ls_path, input_path) != 0) {
        fprintf(stderr, "gsensor_open(): %s\n", gsensor_errmsg(ls));
        return -1;
    }

    if (enable != -1) {
        if (gsensor_set_enable(ls, enable) < 0) {
            fprintf(stderr, "gsensor_set_enable(): %s\n", gsensor_errmsg(ls));
            return -1;
        }      
    }
    ...
    while(read > 0) {
        ...
        if (gsensor_read(ls, &event, read_ev, -1) != read_ev) {
            fprintf(stderr, "gsensor_read(): %s\n", gsensor_errmsg(ls));
            return -1;
        }
    ...
    if (gsensor_close(ls) != 0) {
        fprintf(stderr, "gsensor_close(): %s\n", gsensor_errmsg(ls));
        return -1;
    }
